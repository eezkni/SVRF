...
parser.add_argument("--dump_images",  action='store_true', default=False,
        help='render and store the images')
parser.add_argument('--dataset', type=str, default='syn', choices=['syn', 'tnt', 'mvs', 'nsvf'])
...
def run_exp(env,  config, datadir, expname, basedir):
    ...
    base_cmd = ['python', 'run.py',  '--config', auto_config_path, 
        '--render_test', '--render_fine',
        f'--importance_prune {args.importance_prune}', 
        f'--importance_final {args.importance_final}']
    if not args.if_quantize:
        base_cmd.extend(['--if_quantize','False'])
    ...
    if args.eval:
        base_cmd.extend(['--eval_ssim', '--eval_lpips_alex', '--eval_lpips_vgg'])
