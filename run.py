...
def config_parser():
    ...
    # vq options
    parser.add_argument("--if_quantize", type=bool, default=True,
                    help='if quantize the final model')
    ...
    parser.add_argument("--importance_prune",  type=float,  default=0.999,
            help='quantile threshold for pruned voxels')
    ...
    parser.add_argument("--render_half",  action="store_true", 
            help='rendering half model')
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    test_eps = time.time() - eps_time
    voxels = torch.sum(model.mask_cache.mask).cpu()

    if len(psnrs):
        print('Testing psnr', np.mean(psnrs), '(avg)')
        if eval_ssim and not eval_lpips_vgg and not eval_lpips_alex:
            print('Testing ssim', np.mean(ssims), '(avg)')
            np.savetxt(f'{savedir}/mean.txt', np.asarray([np.mean(psnrs), np.mean(ssims), 0., 0., voxels, test_eps]))
        elif eval_ssim and eval_lpips_vgg and not eval_lpips_alex:
            print('Testing ssim', np.mean(ssims), '(avg)')
            print('Testing lpips (vgg)', np.mean(lpips_vgg), '(avg)')
            np.savetxt(f'{savedir}/mean.txt', np.asarray([np.mean(psnrs), np.mean(ssims), np.mean(lpips_vgg), 0., voxels, test_eps]))
        elif eval_ssim and eval_lpips_vgg and eval_lpips_alex:
            print('Testing ssim', np.mean(ssims), '(avg)')
            print('Testing lpips (vgg)', np.mean(lpips_vgg), '(avg)')
            print('Testing lpips (alex)', np.mean(lpips_alex), '(avg)')
            np.savetxt(f'{savedir}/mean.txt', np.asarray([np.mean(psnrs), np.mean(ssims), np.mean(lpips_vgg), np.mean(lpips_alex), voxels, test_eps]))
        else:
            np.savetxt(f'{savedir}/mean.txt', np.asarray([np.mean(psnrs), 0., 0., 0., voxels, test_eps]))
...
def create_new_model(cfg, cfg_model, cfg_train, xyz_min, xyz_max, stage, coarse_ckpt_path):
    ...
    if cfg.data.ndc:
        ...
        model = dvgo.DirectVoxGO(
            ...
            N_dynamic_iters=cfg_train.N_dynamic_iters,
            **model_kwargs)
    model = model.to(device)
    optimizer = utils.create_optimizer_or_freeze_model(model, cfg_train, global_step=0)
    return model, optimizer

def create_new_model_for_vq(cfg_model, coarse_ckpt_path, strict=False):
...
def compute_entropy_loss(alpha, ray_id, depth_label=None, weight_depth=1.0):
    # To find the unique elements and their frequencies in ray_id
    unique_elements, inverse_indices, counts = torch.unique(ray_id, return_inverse=True, return_counts=True)

    acc = torch.zeros_like(counts, dtype=alpha.dtype)
    acc.index_add_(0, inverse_indices, alpha)

    ray_prob = alpha / (acc[inverse_indices] + 1e-10)

    entropy_ray = -1*ray_prob*torch.log2(ray_prob+1e-10)
    entropy_sum = torch.zeros_like(counts, dtype=alpha.dtype)
    entropy_sum.index_add_(0, inverse_indices, entropy_ray)
    out = torch.mean(entropy_sum)
    
    if depth_label is not None:
        depth_entropy_ray = -1*(depth_label*torch.log2(ray_prob+1e-10)+(1-depth_label)*torch.log2(1-ray_prob+1e-10))
        depth_entropy_sum = torch.zeros_like(counts, dtype=alpha.dtype)
        depth_entropy_sum.index_add_(0, inverse_indices, depth_entropy_ray)
        out += weight_depth*torch.mean(depth_entropy_sum)
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    render_kwargs = {
        ...
        'depth_label': cfg_train.depth_label,
        'weight_surface_distill':cfg_train.weight_surface_distill,
        'entropy_loss_after': cfg_train.entropy_loss_after,
        'entropy_loss_before': cfg_train.entropy_loss_before,
        'depth_entropy_every': cfg_train.depth_entropy_every,
    }
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ### init importance score
        if global_step % cfg_train.N_dynamic_iters == 0:
        ...
        if model.mask_cache is not None and (global_step + 500) % 1000 == 0:
            model.update_occupancy_cache(global_step, cfg_train.cur_thres)
        ...
        if cfg_train.ray_sampler in ['flatten', 'in_maskcache']:
            ...
            sel_b = torch.randint(rgb_tr.shape[0], [cfg_train.N_rand]).to(rgb_tr.device)
            sel_r = torch.randint(rgb_tr.shape[1], [cfg_train.N_rand]).to(rgb_tr.device)
            sel_c = torch.randint(rgb_tr.shape[2], [cfg_train.N_rand]).to(rgb_tr.device)
        ...
        surf_distill_loss = torch.tensor((0.0))
        psnr = utils.mse2psnr(loss.detach())
        if cfg_train.weight_entropy_last > 0:
            pout = render_result['alphainv_last'].clamp(1e-6, 1-1e-6)
            entropy_last_loss = -(pout*torch.log(pout) + (1-pout)*torch.log(1-pout)).mean()
            loss += cfg_train.weight_entropy_last * entropy_last_loss
        if global_step > cfg_train.entropy_loss_after and global_step <= cfg_train.entropy_loss_before and cfg_train.weight_entropy_loss > 0:
            alpha = render_result['raw_alpha']
            ray_id = render_result['ray_id']
            if cfg_train.depth_label and global_step % cfg_train.depth_entropy_every == 0:
                entropy_loss = cfg_train.weight_entropy_loss * compute_entropy_loss(alpha, ray_id, depth_label=render_result['depth_label'], weight_depth=cfg_train.weight_entropy_depth)
                if cfg_train.weight_surface_distill > 0:
                    surf_distill_loss = cfg_train.weight_surface_distill * F.mse_loss(render_result['surf_marched'], target)
                    loss += surf_distill_loss
            else:
                entropy_loss = cfg_train.weight_entropy_loss * compute_entropy_loss(alpha, ray_id, depth_label=None)
            loss += entropy_loss    
        ...
        if global_step%args.i_print==0:
            ...
            tqdm.write(f'scene_rep_reconstruction ({stage}): iter {global_step:6d} / '
                       f'Loss: {loss.item():.9f} /'
                       f'Entropy Loss: {entropy_loss.item():.9f} /'
                       f'Surface Distill Loss: {surf_distill_loss.item():.9f} / '
    ...
    if global_step != -1:
        ...
        print("{} mask's shape:{}, mask's total number:{}".format(stage, model.mask_cache.mask.shape, model.mask_cache.mask.shape[0] * model.mask_cache.mask.shape[1] * model.mask_cache.mask.shape[-1]))
        print("{} mask's true number:{}".format(stage, torch.sum(model.mask_cache.mask)))      

    return model.mask_cache.mask.shape[0] * model.mask_cache.mask.shape[1] * model.mask_cache.mask.shape[-1], torch.sum(model.mask_cache.mask)


def tensor_quantize(args, cfg, cfg_model, xyz_min, xyz_max, data_dict, stage, load_ckpt_path=None):
    # init
    if abs(cfg_model.world_bound_scale - 1) > 1e-9:
        xyz_shift = (xyz_max - xyz_min) * (cfg_model.world_bound_scale - 1) / 2
        xyz_min -= xyz_shift
    ...
    print(f'scene_rep_reconstruction (quantization): reload from {load_ckpt_path}')
    last_ckpt_path = os.path.join(cfg.basedir, cfg.expname, f'vq_last.tar')
    model= create_new_model_for_vq(cfg_model, load_ckpt_path, strict=False)
    
    #=================== Initialize final-importance score  ====================
    stepsize = cfg.fine_model_and_render.stepsize
    render_viewpoints_kwargs = {
    'model': model,
    'ndc': cfg.data.ndc,
    ...
    init_importance(
        ...
        HW=data_dict['HW'][data_dict['i_train']],
        Ks=data_dict['Ks'][data_dict['i_train']],
        savedir=importance_savedir,if_final=True,
        **render_viewpoints_kwargs)

    #=================== Apply final voxel pruning and tensor quantize  ====================
    model.quantize_reformat(args.importance_final, args.if_quantize,
                                    save_path=os.path.join(cfg.basedir, cfg.expname))
    model.update_occupancy_cache(global_step=-1, cur_thres=1)
...
def train(args, cfg, data_dict):
    ...
    if not os.path.exists(os.path.join(cfg.basedir, cfg.expname, f'corse_last.tar')):
        if cfg.coarse_train.N_iters > 0:
            coarse_n_mask, coarse_true_mask = scene_rep_reconstruction(
                    args=args, cfg=cfg,
    ...
    if not os.path.exists(os.path.join(cfg.basedir, cfg.expname, f'fine_last.tar')):
        cfg.fine_train.update(dict(
            cur_thres = args.importance_prune,
        ))
        fine_n_mask, fine_true_mask = scene_rep_reconstruction(
                args=args, cfg=cfg,
        ...
        fine_time = eps_time_str[:]
        print('train: fine detail reconstruction in', eps_time_str)
    else:
        print('train: skip fine optimization')
        fine_time = 0
        fine_n_mask = None
        fine_true_mask = None
    
    ## quantize and saving
    eps_non_vq = time.time()
    tensor_quantize(
        args=args, cfg=cfg,
        cfg_model=cfg.vq_model_and_render, 
        xyz_min=xyz_min_fine, xyz_max=xyz_max_fine,
        data_dict=data_dict, stage='vq',
        load_ckpt_path=os.path.join(cfg.basedir, cfg.expname, f'fine_last.tar'))
    eps_vq = time.time() - eps_non_vq
    vq_time = f'{eps_vq//3600:02.0f}:{eps_vq//60%60:02.0f}:{eps_vq%60:02.0f}'
    print('train: fine quantization in', vq_time)

    eps_time = time.time() - eps_time
    eps_time_str = f'{eps_time//3600:02.0f}:{eps_time//60%60:02.0f}:{eps_time%60:02.0f}'
    print('train: finish (eps time', eps_time_str, ')')
    with open(os.path.join(cfg.basedir, cfg.expname, 'trainlog.txt'), 'a') as file:
        file.write('coarse total number of voxels: {}\n'.format(coarse_n_mask))
        file.write('coarse total number of non-pruned voxels: {}\n'.format(coarse_true_mask))
        file.write('fine total number of voxels: {}\n'.format(fine_n_mask))
        file.write('fine total number of non-pruned voxels: {}\n'.format(fine_true_mask))
        file.write('train: coarse geometry searching in {}\n'.format(coarse_time))
        file.write('train: fine detail reconstruction in {}\n'.format(fine_time))
        file.write('train: vector quantization in {}\n'.format(vq_time))
        file.write('train: finish in {}\n'.format(eps_time_str))

def init_importance(model, render_poses, HW, Ks, ndc, render_kwargs, savedir=None, if_final=False, render_factor=0, ):
    '''Render images for the given viewpoints; run evaluation if gt given.
    '''
    if if_final:
        imp_path = os.path.join(savedir, 'importance_final.pth')
    else:
...
if __name__=='__main__':
    ...
    if args.render_test or args.render_train or args.render_video:
        ...
        if args.if_quantize:
            model_vq.eval()
            model_vq.importance = torch.load( os.path.join(cfg.basedir, cfg.expname, 'importance_final.pth'))
    ...
    if args.render_train:
        ...
        os.makedirs(testsavedir, exist_ok=True)
        print('All results are dumped into', testsavedir)
        rgbs, depths, bgmaps, metrics = render_viewpoints(
                render_poses=data_dict['poses'][data_dict['i_train']],
                HW=data_dict['HW'][data_dict['i_train']],
                Ks=data_dict['Ks'][data_dict['i_train']],
                gt_imgs=[data_dict['images'][i].cpu().numpy() for i in data_dict['i_train']],
